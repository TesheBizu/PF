...
function Experience() {
  ...
  // Helper to split description lines into clean list items
  const renderDescriptionPoints = (desc) => {
    if (!desc) return null;
    // Split by newlines, bullet symbols, or semicolons
    const points = desc
      .split(/\n|•|;/)
      .map((p) => p.trim())
      .filter((p) => p.length > 0);
      ...
      <div className="container">
        ...
        <div className="section-header animate-fadeInUp">
          ...
          <p className="section-desc">
            A chronological timeline of my professional journey, academic background, and technical milestones.
          </p>
        </div>
          ...
          {loading && experiences.length === 0 ? (
            ...
            experiences.map((exp, index) => {
              ...
              return (
                <div 
                  key={exp._id || index} 
                  className={`timeline__item ${isLeft ? 'timeline__item--left' : 'timeline__item--right'} animate-fadeInUp`}
                  style={{ animationDelay: `${index * 0.15}s` }}
                >
                  {/* Timeline node icon: circular logo or type fallback icon */}
